...
<div class="cover-container d-flex w-100 h-100 p-3 mx-auto flex-column">
  ...
  <main class="px-3" style="background-color: white; width:100%; height:800px; color:black">
    <div class="cover-container d-flex w-50 h-28 p-3 mx-auto flex-column " style="border:1px solid;margin-top:156px">
      <form class="row g-3 flex justify-center items-center" action="{{route('category.update',[$category->id])}}"
        method="post">
        @csrf
        <div class="col-md-4">
          <label for="validationCustom01" class="form-label">Ten Danh Muc</label>
          <input type="text" style="width:324px;" class="form-control" value="{{ $category->name }}" name="name"
            placeholder="Nhập tên">
        </div>
        <div class="col-12" style="margin-left: 37px">
          <div class="form-check">
            @if ($category->status == 0)
            <input class="form-check-input -mb-2" name="Activi" checked="checked" style="margin-left: 198px;"
              type="checkbox" value="0" id="invalidCheck">
